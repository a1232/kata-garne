...
def grad_date():
    ...
    write_to_file(selected_date)  # Write the selected date to file
    subprocess.run(["python", "DATE.py", selected_date])  # Pass selected date as argument to DATE.py

def write_to_file(selected_value):
    with open("selected_date.txt", "w") as file:  # Writing selected date to a file named selected_date.txt
